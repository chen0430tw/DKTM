...
class PlatformOps:
    ...
    def commit_transition(self, auto_reboot: bool = False) -> None:
        ...
        if not self.winpe_entry_ids:
            self.logger.warning(
                "No WinPE entry IDs configured; cannot commit transition"
            )
            return
        ...
        if self.marker_path:
            try:
                ...
                self.logger.error(
                    "Failed to write marker file %s: %s", self.marker_path, exc
                )
    ...
    def rollback_transition(self) -> None:
        ...
        if self.marker_path and os.path.isfile(self.marker_path):
            try:
                ...
                self.logger.error(
                    "Failed to remove marker file %s: %s", self.marker_path, exc
                )
